...
export default function VoxReader() {
  ...
  useEffect(() => {
    ...
    const updatePages = () => {
      ...
      if (el) {
        // 使用 getBoundingClientRect 獲取精確寬度
        const viewW = el.getBoundingClientRect().width;
        const totalW = el.scrollWidth;
        
        // 重要：在有 columnGap 的情況下，每頁佔用的滾動寬度是 viewW + columnGap
        // 但最後一頁沒有後面的 Gap，所以計算公式為 (totalW + gap) / (viewW + gap)
        const gap = 40;
        const pages = Math.max(1, Math.round((totalW + gap) / (viewW + gap)));
        setTotalPages(pages);
      }
    };
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-slate-50 font-sans">
        <div className="w-full max-w-md bg-white p-8 rounded-2xl shadow-xl space-y-6">
          ...
          <input type="text" value={docUrl} onChange={(e) => setDocUrl(e.target.value)} placeholder="Google Docs 連結" className="w-full p-3 border rounded-lg text-slate-700 outline-none focus:ring-2 focus:ring-blue-500" />
          <input type="text" value={audioUrl} onChange={(e) => setAudioUrl(e.target.value)} placeholder="音檔連結 (選填)" className="w-full p-3 border rounded-lg text-slate-700 outline-none focus:ring-2 focus:ring-blue-500" />
          <button onClick={handleLoad} disabled={loading} className="w-full bg-blue-600 text-white py-3 rounded-lg font-semibold hover:bg-blue-700 transition">{loading ? '載入中...' : '開始視聽'}</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-stone-100 overflow-hidden">
      ...
      <div className="h-14 flex items-center justify-between px-6 bg-white border-b z-20">
        <button onClick={() => setIsSetup(true)} className="text-slate-500 text-sm hover:text-slate-800 transition">← 返回</button>
        <div className="flex items-center space-x-2">
           <span className="text-[10px] text-slate-400">A</span>
           <input type="range" min="16" max="60" value={fontSize} onChange={(e) => setFontSize(parseInt(e.target.value))} className="w-24 sm:w-32 accent-blue-600 cursor-pointer" />
           <span className="text-lg text-slate-400">A</span>
        </div>
        <div className="text-slate-500 text-xs font-mono tabular-nums">{currentPage + 1} / {totalPages}</div>
      </div>
      ...
      <div className="flex-1 relative bg-stone-100" 
        ...
        <div ref={contentRef} 
             ...
             style={{ 
               display: 'block',
               columnWidth: 'calc(100vw - 40px)', // 重要：考慮到 padding 後的寬度
               columnGap: '40px', 
               columnFill: 'auto',
